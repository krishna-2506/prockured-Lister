...
def _first_visible(locator):
    """Return the first visible locator without relying on screen coordinates."""
    try:
        count = locator.count()
    except Exception:
        return None
...
def _tab_aliases(tab_name: str):
    wanted = str(tab_name or '').strip().lower()
    if wanted in {'basic', 'basics'}:
        return ['Basics', 'Basic']
    if wanted in {'variation', 'variations', 'catalog'}:
        return ['Variations', 'Catalog']
    return [str(tab_name or '').strip()]


def _wait_for_tab_active(tab, timeout_ms: int = 8000) -> bool:
...
def click_tab(page, tab_name):
    """Open a product tab using its DOM role/name, independent of viewport size."""
    aliases = [a for a in _tab_aliases(tab_name) if a]
    log(f"Opening {tab_name} tab using responsive DOM locators...")

    form = page.locator('form.admin-product-form')
    scope = form if form.count() else page

    for alias in aliases:
        pattern = re.compile(rf'^\s*{re.escape(alias)}\s*$', re.I)
        tab = _first_visible(scope.get_by_role('tab', name=pattern))
        if tab is None:
            continue
        try:
            tab.scroll_into_view_if_needed()
            tab.click(timeout=5000)
            if _wait_for_tab_active(tab):
                page.wait_for_timeout(250)
                return True
        except Exception as exc:
            log(f"  Tab locator failed for {alias}: {exc}")

    # Radix tab IDs are dynamic at the start but have a stable suffix.
    suffixes = []
    for alias in aliases:
        lower = alias.lower()
        if lower in {'basic', 'basics'}:
            suffixes.append('basics')
        elif lower in {'variation', 'variations', 'catalog'}:
            suffixes.extend(['catalog', 'variations'])
        else:
            suffixes.append(lower)

    for suffix in dict.fromkeys(suffixes):
        tab = _first_visible(scope.locator(f"[role='tab'][id$='-trigger-{suffix}']"))
        if tab is None:
            continue
        try:
            tab.scroll_into_view_if_needed()
    ...
    # Final text fallback still clicks the element itself, never an x/y touchpoint.
    for alias in aliases:
        candidate = _first_visible(scope.locator("button[role='tab'], button").filter(
            has_text=re.compile(rf'^\s*{re.escape(alias)}\s*$', re.I)
        ))
    ...
    log(f"Could not open tab: {tab_name}")
    return False
...
def _select_product_type_locator(page):
    form = page.locator('form.admin-product-form')
    scope = form if form.count() else page
    labels = scope.locator('label').filter(has_text=re.compile(r'^\s*Product Type\s*\*?\s*$', re.I))
    label = _first_visible(labels)
    if label is not None:
        try:
            select = label.locator('xpath=following::select[1]')
            visible = _first_visible(select)
            if visible is not None:
                return visible
        except Exception:
            pass

    # The Product Type select contains these three stable option values.
    candidates = scope.locator("select:has(option[value='simple']):has(option[value='variable'])")
    return _first_visible(candidates)


def fill_product_type_dropdown(page, product_type: str):
    """Select Product Type from its real HTML select and verify the result."""
    product_type = str(product_type or '').strip()
    if not product_type:
        return False

    requested = product_type.lower()
    if 'variable' in requested:
        value = 'variable'
    elif 'group' in requested or 'bundle' in requested:
        value = 'group'
    else:
        value = 'simple'

    select = _select_product_type_locator(page)
    if select is None:
        log('Product Type select was not found by DOM locator.')
        return False
    ...
    try:
        ...
        ok = actual == value
        log(f"Product Type requested={product_type} selected={actual} match={ok}")
        return ok
    except Exception as exc:
        log(f"Product Type selection failed: {exc}")
        return False


def _fill_visible_field(locator, value: str, field_name: str, clear_only: bool = False) -> bool:
    target = _first_visible(locator)
    if target is None:
        return False
    try:
        target.scroll_into_view_if_needed()
        target.fill('' if clear_only else str(value or ''))
        log(f"  {field_name}: filled")
        return True
    except Exception as exc:
        log(f"  {field_name}: locator fill failed: {exc}")
        return False


def fill_basics(page, data: ProductData):
    """Fill Basics using stable placeholders and the real Product Type select."""
    if not click_tab(page, 'Basics'):
        raise RuntimeError('Basics tab could not be opened')

    b = data.basics or {}
    form = page.locator('form.admin-product-form')
    scope = form if form.count() else page
    log('Filling Basics with responsive DOM locators. Category, Brand and SKU are handled separately.')

    if b.get('Product Type'):
        if not fill_product_type_dropdown(page, b.get('Product Type')):
            raise RuntimeError('Product Type could not be selected')

    if b.get('Product Name'):
        ok = _fill_visible_field(
            scope.get_by_placeholder('Eg. Stainless Steel Fry Basket'),
            b.get('Product Name'), 'Product Name'
        )
        if not ok:
            result = set_field_in_labeled_section(page, ['PRODUCT NAME', 'Product Name'], b.get('Product Name'), False, True)
            ok = bool(result and result.get('ok'))
        if not ok:
            raise RuntimeError('Product Name field could not be filled')

    # Keep the established behavior of leaving Slug empty for auto-generation.
    _fill_visible_field(scope.get_by_placeholder('auto-generated-if-empty'), '', 'Slug', clear_only=True)

    if b.get('Product Tags'):
        ok = _fill_visible_field(
            scope.get_by_placeholder('eco-friendly, best-seller, compostable'),
            b.get('Product Tags'), 'Product Tags'
        )
        if not ok:
            result = set_field_in_labeled_section(page, ['PRODUCT TAGS', 'Product Tags'], b.get('Product Tags'), True, True)
            ok = bool(result and result.get('ok'))
        if not ok:
            raise RuntimeError('Product Tags field could not be filled')

    if b.get('Description'):
        ok = _fill_visible_field(
            scope.get_by_placeholder(re.compile(r'Paste product notes like dimensions', re.I)),
            b.get('Description'), 'Description'
        )
        if not ok:
            result = set_field_in_labeled_section(page, ['DESCRIPTION', 'Description'], b.get('Description'), True, False)
            ok = bool(result and result.get('ok'))
        if not ok:
            raise RuntimeError('Description field could not be filled')

    if b.get('Short Description'):
        ok = _fill_visible_field(
            scope.get_by_placeholder('Short fact notes or summary used in cards and previews'),
            b.get('Short Description'), 'Short Description'
        )
        if not ok:
            result = set_field_in_labeled_section(page, ['SHORT DESCRIPTION', 'Short Description'], b.get('Short Description'), True, False)
            ok = bool(result and result.get('ok'))
        if not ok:
            raise RuntimeError('Short Description field could not be filled')

    if b.get('Model Name'):
        model_name = str(b.get('Model Name') or '').strip()
        ok = _fill_visible_field(
            scope.get_by_placeholder('Optional model identifier'),
            model_name, 'Model Name'
        )
        if not ok:
            result = set_field_in_labeled_section(page, ['MODEL NAME', 'Model Name'], model_name, False, True)
            ok = bool(result and result.get('ok'))
        if not ok:
            raise RuntimeError('Model Name field could not be filled')
...
def _attributes_panel(page):
    panel = page.locator("[role='tabpanel'][aria-labelledby$='trigger-attributes']:not([hidden])")
    visible = _first_visible(panel)
    return visible if visible is not None else page


def _attribute_cards(page):
    panel = _attributes_panel(page)
    return panel.locator(
        "div.grid:has(input[placeholder='Name (e.g. Material)'])"
        ":has(input[placeholder^='Add value and press Enter'])"
    )


def _attribute_card_for_name(page, attribute_name: str):
    wanted = norm_key(attribute_name)
    cards = _attribute_cards(page)
    for index in range(cards.count()):
        card = cards.nth(index)
        try:
            name_input = card.locator("input[placeholder='Name (e.g. Material)']").first
            if norm_key(name_input.input_value()) == wanted:
                return card
        except Exception:
            continue
    return None
...
def _attribute_delete_button(card):
    selectors = [
    ...
    for selector in selectors:
        button = _first_visible(card.locator(selector))
...
def clear_existing_attributes(page, max_clicks: int = 80):
    """Clear attribute cards using card-scoped delete buttons, not screen positions."""
    log('Clearing old Attribute cards with responsive DOM locators...')
    deleted = 0
    for _ in range(max_clicks):
        ...
        target_card = None
        # Prefer deleting a filled card. Keep a single empty reusable card if the UI creates one.
        for index in range(count):
            card = cards.nth(index)
            try:
                value = card.locator("input[placeholder='Name (e.g. Material)']").first.input_value().strip()
            except Exception:
                value = ''
            if value or count > 1:
                target_card = card
        ...
        button = _attribute_delete_button(target_card)
        if button is None:
            log('  Attribute delete button was not found inside the card.')
            break
        try:
            ...
            log(f'  Attribute deletion failed: {exc}')
            break
    log(f'Deleted attribute cards: {deleted}')
...
def set_attribute_toggle_by_name(page, attribute_name: str, toggle_name: str = 'Variants', desired: bool = True) -> bool:
    """Set Filters/Variants in the exact attribute card with set_checked()."""
    card = _attribute_card_for_name(page, attribute_name)
    if card is None:
        log(f"  Toggle failed: attribute card not found: {attribute_name}")
        return False

    label = _first_visible(card.locator('label').filter(
        has_text=re.compile(rf'^\s*{re.escape(toggle_name)}\s*$', re.I)
    ))
    if label is None:
        log(f"  Toggle label not found: {attribute_name} → {toggle_name}")
        return False

    checkbox = _first_visible(label.locator("input[type='checkbox']"))
    if checkbox is None:
        log(f"  Checkbox not found: {attribute_name} → {toggle_name}")
        return False

    try:
        before = checkbox.is_checked()
        if before != bool(desired):
            checkbox.set_checked(bool(desired), force=True)
            page.wait_for_timeout(180)
        after = checkbox.is_checked()
        ok = after == bool(desired)
        log(f"  {attribute_name} → {toggle_name}: before={before} after={after} desired={desired} match={ok}")
        return ok
    except Exception as exc:
        log(f"  Toggle failed for {attribute_name} → {toggle_name}: {exc}")
        return False
...
def ensure_variant_attributes_checked(page, data: ProductData):
    """Verify the exact Filters and Variants state for every classified attribute."""
    if not data:
        return True

    desired = {}
    def merge(pairs, wants_filter, wants_variant):
        for name, _value in pairs:
            key = norm_key(name)
            item = desired.setdefault(key, {'name': name, 'filter': False, 'variant': False})
            item['filter'] = item['filter'] or wants_filter
            item['variant'] = item['variant'] or wants_variant

    merge(data.attributes, False, False)
    merge(data.filter_attributes, True, False)
    merge(data.variant_attributes, False, True)
    merge(data.variant_filter_attributes, True, True)

    ok = True
    log('Verifying all attribute Filters/Variants states...')
    for item in desired.values():
        if stop_requested:
            return False
        ok = set_attribute_toggle_by_name(page, item['name'], 'Filters', item['filter']) and ok
        ok = set_attribute_toggle_by_name(page, item['name'], 'Variants', item['variant']) and ok
    log(f'Attribute checkbox verification complete: {ok}')
    return ok
...
def _empty_attribute_name_input(page):
    cards = _attribute_cards(page)
    for index in range(cards.count()):
        card = cards.nth(index)
        try:
...
def fill_one_attribute(
    ...
    """Fill one attribute entirely inside its own DOM card."""
    name_input, card = _empty_attribute_name_input(page)
    if name_input is None:
        before = _attribute_cards(page).count()
        click_add_attribute(page)
        deadline = time.time() + 5
        while time.time() < deadline and _attribute_cards(page).count() <= before:
            time.sleep(0.1)
        name_input, card = _empty_attribute_name_input(page)

    if name_input is None or card is None:
        log('  Could not find or create an empty Attribute card.')
        return False
    ...
    log(f"Attribute → {name} : {value}" + (f" [{' + '.join(flags)}]" if flags else ''))
    ...
    try:
        ...
        log(f'  Attribute Name fill failed: {exc}')
        return False

    # Re-resolve after React rerenders the card.
    card = _attribute_card_for_name(page, name)
    if card is None:
        log('  Attribute card could not be re-resolved after entering the name.')
        return False

    value_input = _first_visible(card.locator("input[placeholder^='Add value and press Enter']"))
    if value_input is None:
        log('  Value input was not found inside the attribute card.')
        return False

    values = split_variant_values(value) if split_values else [str(value or '').strip()]
    try:
        for one_value in values:
            if not one_value:
                continue
            value_input.click(timeout=5000)
            value_input.fill('')
            value_input.fill(one_value)
            if press_enter:
                value_input.press('Enter')
            page.wait_for_timeout(220)
    except Exception as exc:
        log(f'  Attribute value fill failed: {exc}')
        return False

    # Explicitly set both states. This handles all four JSON attribute classes.
    filter_ok = set_attribute_toggle_by_name(page, name, 'Filters', bool(is_filter))
    variant_ok = set_attribute_toggle_by_name(page, name, 'Variants', bool(is_variant))
    return filter_ok and variant_ok

def remove_empty_attribute_blocks(page, max_remove: int = 20):
    """Remove extra empty cards while preserving one reusable empty card."""
    removed = 0
    for _ in range(max_remove):
        ...
        for index in range(cards.count()):
            card = cards.nth(index)
            try:
                name_value = card.locator("input[placeholder='Name (e.g. Material)']").first.input_value().strip()
                chip_buttons = card.locator("button[aria-label^='Remove ']")
                typed_value = card.locator("input[placeholder^='Add value and press Enter']").first.input_value().strip()
                if not name_value and not typed_value and chip_buttons.count() == 0:
                    empty_cards.append(card)
            except Exception:
                continue
        if len(empty_cards) <= 1:
            break
        button = _attribute_delete_button(empty_cards[-1])
        if button is None:
            break
        try:
            button.click(timeout=5000)
            ...
            break
    log(f'Removed extra empty Attribute cards: {removed}')

def fill_attributes(page, data: ProductData, one=False, clear=True):
    """Fill normal, filter, variant, and filter+variant attribute classes."""
    merged = {}
    ...
    def add_pairs(pairs, wants_filter=False, wants_variant=False, split_values=False):
        for name, raw_value in pairs:
            ...
            item = merged.setdefault(key, {
                'name': name, 'values': [], 'is_filter': False,
                'is_variant': False, 'split_values': False,
            })
            ...
            for incoming_value in incoming:
                if incoming_value and norm_key(incoming_value) not in seen:
                    item['values'].append(incoming_value)
                    seen.add(norm_key(incoming_value))
    ...
    if not combined:
        ...
        return True
    if not click_tab(page, 'Attributes'):
        return False
    page.wait_for_timeout(400)

    attrs = combined[:1] if one else combined
    log(
        f"Attributes: normal={len(data.attributes)} filter={len(data.filter_attributes)} "
        f"variant={len(data.variant_attributes)} variant+filter={len(data.variant_filter_attributes)} "
        f"merged={len(attrs)}"
    )
    ...
    for index, item in enumerate(attrs, start=1):
        if stop_requested:
            return False
        if not fill_one_attribute(
            page, item['name'], item['value'],
            press_enter=PRESS_ENTER_FOR_ATTRIBUTE_VALUE,
            is_variant=item['is_variant'], is_filter=item['is_filter'],
            split_values=item['split_values'],
        ):
            log(f"Attribute fill failed at {index}/{len(attrs)}: {item['name']}")
            return False

    remove_empty_attribute_blocks(page)
    return ensure_variant_attributes_checked(page, data)

def _variant_card_count(page) -> int:
    """Count generated variant cards from DOM content, independent of their position."""
    try:
        return int(page.evaluate(
            r"""() => {
                ...
                const panel = [...document.querySelectorAll('[role="tabpanel"]')]
                    .find(el => visible(el) && /product variants|variant axes|generated variants/i.test(el.innerText || ''));
                if (!panel) return 0;
                const labels = [...panel.querySelectorAll('label, p, span, div')]
                    .filter(visible)
                    .filter(el => /regular price|base price/i.test((el.innerText || el.textContent || '').trim()));
                const cards = new Set();
                for (const label of labels) {
                    ...
                    for (let d = 0; node && d < 10; d++, node = node.parentElement) {
                        ...
                        const inputs = [...node.querySelectorAll('input')].filter(visible);
                        if (inputs.length >= 2 && text.includes('active') && node.getBoundingClientRect().height < 800) {
                            cards.add(node);
                            break;
                        }
                    }
...
def click_generate_variants(page):
    """Generate only when variant cards do not already exist, then verify creation."""
    if not click_tab(page, 'Variations'):
        if not click_tab(page, 'Catalog'):
            return False
    page.wait_for_timeout(600)

    existing = _variant_card_count(page)
    if existing > 0:
        log(f'Existing generated variant cards found: {existing}. Regeneration skipped.')
        return True

    button = _first_visible(page.get_by_role('button', name=re.compile(r'^\s*Generate Variants\s*$', re.I)))
    if button is None:
        # If the empty message is gone, rows may be rendering; wait briefly.
        log('Generate Variants button not found. Waiting for existing rows to render...')
        deadline = time.time() + 8
        while time.time() < deadline:
            if _variant_card_count(page) > 0:
                return True
            time.sleep(0.25)
        return False
    ...
    try:
        button.scroll_into_view_if_needed()
        button.click(timeout=5000)
        log('Generate Variants clicked. Waiting for generated cards...')
    except Exception as exc:
        log(f'Generate Variants click failed: {exc}')
        return False

    deadline = time.time() + 20
    while time.time() < deadline:
        count = _variant_card_count(page)
        if count > 0:
            log(f'Generated variant cards confirmed: {count}')
            return True
        time.sleep(0.3)

    log('Variant generation could not be confirmed within 20 seconds.')
    return False
...
def fill_variations(page, data: ProductData):
    if not is_variable_product(data):
        ...
        return True
    if not click_generate_variants(page):
        raise RuntimeError('Variants were not generated and no existing variant cards were found')
    fill_variant_prices(page, data)
    count = _variant_card_count(page)
    if count <= 0:
        raise RuntimeError('No generated variant cards were found after variation processing')
    log(f'Variation workflow complete. Variant cards visible: {count}')
    return True
...
def _type_category_and_press_enter(page, search_text: str):
    """Preserve the proven Enter → Tab → Tab commit sequence using the Category element itself."""
    search_text = str(search_text or '').strip()
    if not search_text:
        return {'ok': False, 'reason': 'empty-search-text'}

    field = _first_visible(page.get_by_placeholder('Search category or sub-category...'))
    if field is None:
        field = _first_visible(page.get_by_placeholder(re.compile(r'search category or sub-category', re.I)))
    if field is None:
        return {'ok': False, 'reason': 'category-input-not-found'}
    ...
    try:
        ...
        return {
            'ok': True,
            'method': 'locator-fill-enter-tab-tab',
            'search_text': search_text,
            'selected_after': _read_selected_category_text(page),
        }
    except Exception as exc:
        return {'ok': False, 'reason': str(exc), 'method': 'locator-fill-enter-tab-tab'}

def _read_selected_category_text(page) -> str:
    """Read the live Category textbox value without position assumptions."""
    try:
        field = _first_visible(page.get_by_placeholder('Search category or sub-category...'))
        if field is None:
            field = _first_visible(page.get_by_placeholder(re.compile(r'search category or sub-category', re.I)))
        return str(field.input_value() if field is not None else '').strip()
    except Exception:
        return ''
...
def _read_publish_summary_value(page, summary_label: str) -> str:
    """Read Category/Brand/SKU from the Publish summary card."""
    pattern = re.compile(rf'^\s*{re.escape(summary_label)}\s*$', re.I)
    labels = page.locator('p').filter(has_text=pattern)
    for index in range(labels.count()):
        label = labels.nth(index)
        try:
            if not label.is_visible():
                continue
            row = label.locator('xpath=parent::*[1]')
            values = row.locator('p')
            if values.count() >= 2:
                return str(values.nth(1).inner_text()).strip()
        except Exception:
            continue
    return ''


def _category_is_committed(page, requested: str) -> tuple[bool, str]:
    requested_leaf = category_search_text(requested)
    input_value = _read_selected_category_text(page)
    summary_value = _read_publish_summary_value(page, 'Category')
    requested_norm = normalize_select_text(requested_leaf)
    input_norm = normalize_select_text(input_value)
    summary_norm = normalize_select_text(summary_value)

    input_match = bool(requested_norm and input_norm and requested_norm in input_norm)
    summary_match = bool(
        summary_value
        and 'no category selected' not in summary_value.lower()
        and (requested_norm in summary_norm or summary_norm in normalize_select_text(requested))
    )
    # The Publish summary is the strongest proof that the category ID was committed.
    if summary_value:
        return summary_match, f"input='{input_value}' summary='{summary_value}'"

    # Fallback only when the summary card cannot be read: require both the visible
    # value and the old proven focus transition to Brand after Enter → Tab → Tab.
    focus = _active_focus_is_brand_field(page)
    focus_ok = bool(focus and focus.get('ok'))
    return input_match and focus_ok, f"input='{input_value}' summary=unavailable brand_focus={focus_ok}"


def select_category_for_batch(page, category_option: str) -> bool:
    """Select and commit Category; direct option click first, proven keyboard fallback second."""
    category_option = str(category_option or '').strip()
    if not category_option:
        return False
    search = category_search_text(category_option)
    log(f"Selecting mandatory Category: option='{category_option}' search='{search}'")
    ...
    if field is None:
        field = _first_visible(page.get_by_placeholder(re.compile(r'search category or sub-category', re.I)))
    if field is None:
        log('  Category input was not found.')
        return False

    # Preferred path: type, then click the actual dropdown result beneath Category.
    try:
        field.scroll_into_view_if_needed()
        field.click(timeout=5000)
        field.fill('')
        field.fill(search)
        page.wait_for_timeout(900)
        clicked = _click_category_dropdown_result(page, category_option, search_text=search)
        log(f"  Category dropdown option click: {clicked}")
        if clicked:
            page.wait_for_timeout(500)
            page.keyboard.press('Tab')
            page.wait_for_timeout(150)
            page.keyboard.press('Tab')
            page.wait_for_timeout(500)
            ok, details = _category_is_committed(page, category_option)
            log(f"  Category verification after option click: {ok} | {details}")
            if ok:
                return True
    except Exception as exc:
        log(f"  Category option-click path failed: {exc}")

    # Required compatibility fallback from the previous working script.
    result = _type_category_and_press_enter(page, search)
    log(f"  Category Enter+Tab+Tab fallback result: {result}")
    if not result or not result.get('ok'):
        return False

    ok, details = _category_is_committed(page, category_option)
    log(f"  Category verification after keyboard fallback: {ok} | {details}")
    return ok
...
def click_update_product(page) -> bool:
    """Click the submit button inside the product form by its accessible name."""
    log('Clicking Update Product using form-scoped DOM locator...')
    form = page.locator('form.admin-product-form')
    scope = form if form.count() else page
    patterns = [
        re.compile(r'^\s*Update Product\s*$', re.I),
        re.compile(r'^\s*Publish Product\s*$', re.I),
        re.compile(r'^\s*Save Product\s*$', re.I),
        re.compile(r'^\s*Save\s*$', re.I),
    ]
    for pattern in patterns:
        ...
        try:
            ...
            if not button.is_enabled():
                log(f"  Save button is disabled: {button.inner_text()}")
                return False
            button.click(timeout=5000)
            page.wait_for_timeout(700)
            return True
        except Exception as exc:
            log(f'  Save button click failed: {exc}')
    return False
...
def run_batch_json(
    ...
    batch_logger.write("========================================")
    batch_logger.write("Prockured Listing CLI V2 Responsive batch started")
...
def print_cli_and_hotkey_banner(args=None, run_options: BatchRunOptions | None = None):
    log('============================================================')
    log('Prockured Independent Listing Bot - CLI V2.0 Responsive')
    log('Made By Krishna Maheshwari')
    log('============================================================')
    log('RESPONSIVE AUTOMATION')
    log('  Uses DOM roles, placeholders, labels and card-scoped controls.')
    log('  No fixed laptop touchpoints are used for tabs, Basics, Category,')
    log('  Attribute Filters/Variants, Generate Variants or Update Product.')
    log('  Category keeps the proven direct-select and Enter → Tab → Tab fallback.')
    log('')
    log('BATCH CLI')
    ...
    log('  --variation sets Variable Product, recreates classified attributes,')
    log('    verifies Filters/Variants, generates variants only when missing,')
    log('    fills prices and never changes the Active checkbox.')
    log('')
    log('Examples:')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json --media')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json --attributes --seo')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json --variation')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json --lookup modelnumber')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json --media --forceMedia "D:\\Product Images" --MediaSort modelnumber')
    log('  py independent_listing_bot_cli_v2_responsive.py --batch products.json --resume batch_reports\\LAST_RUN\\success_report.csv')
    log('')
    log('MediaSort: modelnumber | sku | name | brand/modelnumber | brand/sku | brand/name')
    log('JSON attributes: attributes | filter_attributes | variant_attributes | variant_filter_attributes')
    log('============================================================')
    ...
    log('Alt + Shift + J  = Run default Batch JSON')
...
def main():
    parser = argparse.ArgumentParser(
        description="Prockured Independent Listing Bot CLI V2 Responsive",
    ...
    parser.add_argument("--version", action="version", version="Prockured Listing Bot CLI V2.0 Responsive")
